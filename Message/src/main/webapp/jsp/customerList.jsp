			...
			success: function(map){
				...
				for(var i = 0; i<map["list"].length; i++ ){
					...
					if(map["list"][i].checkStatus!=0){
						...
						lala += "<a class='modify' href='/Message/selectById2?id="+map["list"][i].id+"'>修改</a>"
		...
		$.ajax({
			...
			success: function(map){
				...
				var page = ""
					//首页
					page += "<ul>"
						page += "	<li class='im-indexpage'><a data-id='' onclick='b(this.id)' id ='1'  title=''>首页</a></li>"
					//上一页
				if(map.pageNum>1){
					page += "	<li class='im-nextpage'><a data-id='' onclick='b(this.id)' id ='"+(parseInt(map.pageNum)-1)+"' title=''>上一页</a></li>"
				}else{
					page += "	<li class='im-nextpage'><a data-id='' onclick='b(this.id)' id ='1'  title=''>上一页</a></li>"
				}	
					//下一页
				if(map.pageNum<map.pageSize){
					page += "	<li class='im-nextpage'><a data-id='' onclick='b(this.id)' id ='"+(parseInt(map.pageNum)+1)+"' title=''>下一页</a></li>"
				}else{
					page += "	<li class='im-nextpage'><a data-id='' onclick='b(this.id)' id ='"+parseInt(map.pageSize)+"' title=''>下一页</a></li>"
				}	
					//尾页
				page += "	<li class='im-indexpage'><a data-id='' onclick='b(this.id)' id ='"+parseInt(map.pageSize)+"' title=''>尾页</a></li>"
				page += "</ul>"
				$(".im-page").append(page);	
			}
		})
...
<body>
	...
	<div class="cus-content">
		...
		<div class="cus-search">
			...
			<a onclick="fuzzyselect()" class="search-btn">搜索</a>
		</div>
